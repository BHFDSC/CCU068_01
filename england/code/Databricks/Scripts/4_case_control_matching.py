...
CHD_cases =  spark.table(f'.ccu068_cases_resolved_{window}').distinct()
CHD_controls = spark.table(f'.ccu068_controls_resolved_{window}').distinct()
...
matched_cases.distinct().write.mode('overwrite').option('overwriteSchema', 'True').saveAsTable(f'.ccu068_cases_matched_personal_info_{window}')


# COMMAND ----------

matched_controls.distinct().write.mode('overwrite').option('overwriteSchema', 'True').saveAsTable(f'.ccu068_controls_matched_personal_info_{window}')

# COMMAND ----------

matched_cases = spark.table(f'.ccu068_cases_matched_personal_info_{window}')
matched_controls = spark.table(f'.ccu068_controls_matched_personal_info_{window}')
