...
inclusion_codes = spark.table('hive_metastore..ccu068_code_dict')
...
snomed_exclusion_codes = spark.table('hive_metastore..chd_exclusion_snomed').select('snomed_ct_concept_id').withColumnRenamed('snomed_ct_concept_id','code').withColumn('code_type', F.lit('snom'))
icd10_exclusion_codes = spark.table('hive_metastore..icd_confounder_codes').select('icd_10').withColumnRenamed('icd_10','code').withColumn('code_type', F.lit('diag'))
opcs4_exclusion_codes = spark.table('hive_metastore..opcs_confounder_codes').select('opcs_4_code').withColumnRenamed('opcs_4_code','code').withColumn('code_type', F.lit('oper'))
...
death_register = spark.table('hive_metastore..deaths_')
...
gdppr_data = spark.table('hive_metastore..gdppr_')
...
hes_apc = spark.table('._hes_apc_all_years')
hes_apc = (
    hes_apc
...
hes_op = spark.table('._hes_op_all_years')
hes_op = (
    hes_op
        .drop(F.col('DOB'))
...
pre_filtered_CHD_records.write.mode('overwrite').saveAsTable(f'.temp_CHD_records_ccu068_{window}')
pre_filtered = spark.table(f'.temp_CHD_records_ccu068_{window}').distinct()
...
gdppr_controls.write.mode('overwrite').saveAsTable(f'.gdppr_controls_ccu068_{window}')
hes_apc_controls.write.mode('overwrite').saveAsTable(f'.hes_apc_controls_ccu068_{window}')
hes_op_controls.write.mode('overwrite').saveAsTable(f'.hes_op_controls_ccu068_{window}')

CHD_records.write.mode('overwrite').saveAsTable(f'.ccu068_cases_chd_records_{window}')
...
cases_all_records = (
    spark.table(f'.ccu068_cases_chd_records_{window}')
    .select('NHS_number')
    .join(pre_filtered, on = 'NHS_number', how = 'inner')
    .drop('dataset')
)
cases_all_records.write.mode('overwrite').saveAsTable(f'.ccu068_cases_all_records_{window}')
...
CHD_controls_final.write.mode('overwrite').saveAsTable(f'.ccu068_controls_all_records_{window}')