...
cases = spark.table(f'.ccu068_{group}_covid_records_unmatched_complication')
skinny = spark.table('.__skinny_record').select('NHS_NUMBER_DEID', 'DATE_OF_BIRTH', 'SEX').withColumnRenamed('NHS_Number_DEID', 'NHS_NUMBER').withColumn('age', F.when(2020-F.year(F.col('Date_of_birth')) >50, '>50').otherwise('<50'))
cases = cases.join(skinny, on = 'NHS_number', how = 'inner')

# COMMAND ----------

thrombotic_snomed_codes = spark.table('.snomed_code_table_for_thrombotic_episodes')
...
carditis_icd10 = spark.table('.ccu068_myopericarditis_icd10').unionByName(endocarditis_icd10)
