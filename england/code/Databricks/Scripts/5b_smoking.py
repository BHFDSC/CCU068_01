...
chd_cases = spark.table(f'.ccu068_cases_matched_personal_info_{window}').select('NHS_number').distinct()
controls = spark.table(f'.ccu068_controls_matched_personal_info_{window}').select('NHS_number').distinct()
gdppr = spark.table('hive_metastore..gdppr_').select(F.col('NHS_NUMBER_DEID').alias('NHS_number'), 'code', 'record_date')
...
smoking_status_cases.write.mode('overwrite').option('overwriteSchema', 'True').saveAsTable(f'.ccu068_cases_matched_smoking_{window}')
smoking_status_controls.write.mode('overwrite').option('overwriteSchema', 'True').saveAsTable(f'.ccu068_controls_matched_smoking_{window}')