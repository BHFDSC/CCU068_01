...
CHD_cases =  spark.table(f'.ccu068_cases_all_records_{window}').select('NHS_number').distinct()

CHD_controls = spark.table(f'.ccu068_controls_all_records_{window}').select('NHS_number').distinct()
...
#1
covid_PCR_tests = spark.table('.sgss_')

#2
covid_hospitalisations_chess = spark.table('.chess_')
...
hes_19_to_22 = spark.table('.hes_apc_1920_').select('PERSON_ID_DEID', 'EPISTART', 'DISDEST', 'DISMETH', 'DIAG_4_01', 'SUSRECID')

for i in range(20,21):
    hes_19_to_22 = hes_19_to_22.unionByName(spark.table(f'.hes_apc_{i}{i+1}_').select('PERSON_ID_DEID', 'EPISTART', 'DISDEST', 'DISMETH', 'DIAG_4_01', 'SUSRECID'))
...
hes_cc_19_to_22 = spark.table('.hes_cc_1920_').select('PERSON_ID_DEID', 'ADMIDATE', 'SUSRECID')

for i in range(20,21):
    hes_cc_19_to_22 = (
        hes_cc_19_to_22
        .unionByName(spark.table(f'.hes_cc_{i}{i+1}_')
...
if window == 'post_vaccination':
    ...
    vaccination_records = spark.table('.vaccine_status_').select('person_id_deid', 'recorded_date').withColumnRenamed('person_id_deid', 'NHS_number')
...
vaccination_records = spark.table('.vaccine_status_').select('recorded_date', 'person_id_deid').withColumnRenamed('person_id_deid', 'NHS_number')
...
CHD_cases_covid_outcomes.write.mode('overwrite').option('overwriteSchema', 'True').saveAsTable(f'.ccu068_cases_covid_records_unmatched_{window}')
CHD_controls_covid_outcomes.write.mode('overwrite').option('overwriteSchema', 'True').saveAsTable(f'.ccu068_controls_covid_records_unmatched_{window}')
