...
CHD_cases =  spark.table(f'.ccu068_cases_covid_records_unmatched_{window}').select('NHS_number').distinct().join(spark.table(f'.ccu068_cases_all_records_{window}'), on = 'NHS_number', how = 'inner')
CHD_controls = spark.table(f'.ccu068_controls_covid_records_unmatched_{window}').select('NHS_number').distinct().join(spark.table(f'.ccu068_controls_all_records_{window}').select('NHS_number', 'gp_practice', 'ethnicity', 'sex', 'DOB', 'record_date'), on = 'NHS_number', how = 'inner')
...
final_CHD_cases.write.mode('overwrite').saveAsTable(f'.ccu068_cases_resolved_{window}')


# COMMAND ----------

final_CHD_controls.write.mode('overwrite').saveAsTable(f'.ccu068_controls_resolved_{window}')

# COMMAND ----------

display(spark.table('.ccu068_cases_resolved_post_vaccination'))