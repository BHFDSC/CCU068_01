...
code_abbrev_dict = spark.table('.ccu068_code_dict')
...
CHD_records = spark.table(f'.ccu068_cases_chd_records_{window}').select(['NHS_number', 'code', 'code_type']).distinct()      
matched_CHD_IDs =  spark.table(f'.ccu068_cases_matched_personal_info_{window}').select('NHS_number').distinct()
...
CHD_all_records = spark.table(f'.ccu068_cases_all_records_{window}').select(['NHS_number', 'code', 'code_type']).distinct()      
matched_CHD_IDs =  spark.table(f'.ccu068_cases_matched_personal_info_{window}').select('NHS_number').distinct()
...
CHD_case_severity.write.mode('overwrite').option('overwriteSchema', 'True').saveAsTable(f'.ccu068_cases_matched_severity_{window}')

# COMMAND ----------

CHD_case_severity = spark.table(f'.ccu068_cases_matched_severity_{window}')
