...
personal_info_CHD_cases <- tbl(con, in_catalog("hive_metastore", "dsa_391419_j3w9t_collab", "ccu068_cases_matched_personal_info_pre_vaccination")) %>% 
  select(NHS_number, ethnicity, sex, DOB, gp_practice)
severity_CHD_cases <- tbl(con, in_catalog("hive_metastore", "dsa_391419_j3w9t_collab", "ccu068_cases_matched_severity_pre_vaccination")) %>%
  select(NHS_number, severity_score, pulmonary_hypertension)
CHD_cases_covid <- tbl(con, in_catalog("hive_metastore", "dsa_391419_j3w9t_collab", "ccu068_cases_matched_covid_pre_vaccination"))

smoking_CHD_cases <- tbl(con, in_catalog("hive_metastore", "dsa_391419_j3w9t_collab", "ccu068_cases_matched_smoking_pre_vaccination")) %>%
...
personal_info_controls <- tbl(con, in_catalog("hive_metastore", "dsa_391419_j3w9t_collab", "ccu068_controls_matched_personal_info_pre_vaccination")) %>% 
  select(NHS_number, ethnicity, sex, DOB, gp_practice)

controls_covid <- tbl(con, in_catalog("hive_metastore", "dsa_391419_j3w9t_collab", "ccu068_controls_matched_covid_pre_vaccination"))
controls_smoking <- tbl(con, in_catalog("hive_metastore", "dsa_391419_j3w9t_collab", "ccu068_controls_matched_smoking_pre_vaccination")) 
...
covid_outcomes <- CHD_cases %>% union(CHD_controls) %>%
  ...
  mutate(DOB = cut(2020 - as.numeric(format(DOB, "%Y")), breaks = 26))
